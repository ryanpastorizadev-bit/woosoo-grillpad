...
onMounted(async () => {
  if (menu.packages.length > 0) return
  try {
...
<template>
  <section class="mx-auto max-w-6xl py-10">
    <h1 class="text-5xl font-black">Choose Package</h1>
    ...
    <div class="mt-8 grid grid-cols-2 gap-6">
      <article v-for="pkg in menu.packages" :key="pkg.id" class="gp-card p-8">
        ...
        <p class="mt-3 text-white/60">{{ pkg.description }}</p>
        <p class="mt-6 text-4xl font-black text-primary">₱{{ pkg.price }}</p>
        <AppButton class="mt-8 w-full" size="lg" :disabled="submitting" @click="choose(pkg.id)">Select</AppButton>
      </article>
    </div>
    <p v-if="!menu.loading && menu.packages.length === 0" class="mt-6 text-sm text-white/60">No packages available for this session.</p>
  </section>
</template>
