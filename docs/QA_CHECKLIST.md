...
- [ ] `/order/refill` only works during `refill` and only shows refill-eligible items (items with refill availability enabled; no initial-only items)
...
- [ ] Refresh restores session state from local storage when session data is present
- [ ] Restored session is confirmed by successful `/session/current` verification when backend session is still valid
