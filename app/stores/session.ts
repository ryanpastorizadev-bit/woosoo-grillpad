...
export const useSessionStore = defineStore('session', {
  ...
  actions: {
    ...
    backToInitialOrder() {
      if (this.phase !== 'review') throw new Error('Can only return to initial order from review phase.')
      this.$patch({ phase: 'initial_order' })
      persistSessionState(this.$state)
    },
    enterRefillMode(orderId: string) {
      if (!this.sessionId || !this.packageId) throw new Error('Cannot enter refill mode without an active packaged session.')
    ...
    markInitialOrderSubmitted(orderId: string) {
      if (this.phase !== 'review') throw new Error('Initial order can only be submitted from review phase.')
      this.$patch({ phase: 'refill', initialOrderId: orderId, initialOrderSubmittedAt: new Date().toISOString() })
      persistSessionState(this.$state)
    },
